...
public class ColorPickerDialog extends Dialog implements
    ...
    public ColorPickerDialog(Context context, int theme, int initialColor,
            int androidColor, int temasekColor) {
        super(context, theme);
        mInitialColor = initialColor;
        mAndroidColor = androidColor;
        mTemasekColor = temasekColor;
        if (mAndroidColor != 0x00000000 && mTemasekColor != 0x00000000) {
    ...
    public boolean onMenuItemClick(MenuItem item) {
        if (item.getItemId() == R.id.palette_temasek) {
            mPalette = PALETTE_TEMASEK;
            mColorTransitionAnimator.start();
            return true;
            ...
            return true;
        } else if (item.getItemId() == R.id.reset_temasek) {
            mColorPicker.setColor(mTemasekColor, true);
            return true;
        }
        return false;
    }
